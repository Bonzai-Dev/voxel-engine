...
namespace boost {
  namespace fibers {
    ...
    class BOOST_FIBERS_DECL context {
      private:
        ...
        struct fss_data {
          ...
          fss_data(void *vp_,
                   detail::fss_cleanup_function::ptr_t fn) noexcept: vp(vp_),
    ...
    inline bool operator==(context const &lhs, context const &rhs) noexcept {
      return &lhs == &rhs;
    }
...
namespace std {
  ...
  struct hash<::boost::fibers::context::id> {
    std::size_t operator()(::boost::fibers::context::id const &id) const noexcept;
  };
}
