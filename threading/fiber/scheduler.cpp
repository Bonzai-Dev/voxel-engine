...
#include "core/threading/context/fiber_fcontext.hpp"
#include "detail/spinlock.hpp"

namespace boost::fibers {
  void scheduler::release_terminated_() noexcept {
    while (!terminated_queue_.empty()) {
      context *ctx = &terminated_queue_.front();
      terminated_queue_.pop_front();
      ENGINE_ASSERT(ctx->is_context(type::worker_context), "");
      ENGINE_ASSERT(!ctx->is_context(type::pinned_context), "");
      ENGINE_ASSERT(this == ctx->get_scheduler(), "");
      ENGINE_ASSERT(ctx->is_resumable(), "");
      ENGINE_ASSERT(!ctx->worker_is_linked(), "");
      ENGINE_ASSERT(!ctx->ready_is_linked(), "");
      ENGINE_ASSERT(!ctx->remote_ready_is_linked(), "");
      ENGINE_ASSERT(!ctx->sleep_is_linked(), "");
      ENGINE_ASSERT(ctx->wait_queue_.empty(), "");
      ENGINE_ASSERT(ctx->terminated_, "");
  ...
  void scheduler::remote_ready2ready_() noexcept {
  ...
  void scheduler::sleep2ready_() noexcept {
    ...
    for (sleep_queue_type::iterator i = sleep_queue_.begin(); i != e;) {
      ...
      ENGINE_ASSERT(!ctx->is_context(type::dispatcher_context), "");
      ENGINE_ASSERT(main_ctx_ == ctx || ctx->worker_is_linked(), "");
      ENGINE_ASSERT(!ctx->ready_is_linked(), "");
      ...
      // outdated
      ENGINE_ASSERT(!ctx->terminated_is_linked(), "");
  ...
  scheduler::~scheduler() {
    ENGINE_ASSERT(nullptr != main_ctx_, "");
    ENGINE_ASSERT(nullptr != dispatcher_ctx_.get(), "");
    ENGINE_ASSERT(context::active() == main_ctx_, "");
    ...
    ENGINE_ASSERT(worker_queue_.empty(), "");
    ENGINE_ASSERT(terminated_queue_.empty(), "");
    ENGINE_ASSERT(sleep_queue_.empty(), "");
    // set active context to nullptr
    context::reset_active();
    // deallocate dispatcher-context
    ENGINE_ASSERT(!dispatcher_ctx_->ready_is_linked(), "");
    dispatcher_ctx_.reset();
    // set main-context to nullptr
    main_ctx_ = nullptr;
  }

  boost::context::fiber scheduler::dispatch() noexcept {
    ENGINE_ASSERT(context::active() == dispatcher_ctx_, "");
    for (;;) {
      if (shutdown_) {
      ...
      if (nullptr != ctx) {
        ...
        ENGINE_ASSERT(context::active() == dispatcher_ctx_.get(), "");
      }
      else {
  ...
  boost::context::fiber scheduler::terminate(detail::spinlock_lock &lk, context *ctx) noexcept {
    ENGINE_ASSERT(nullptr != ctx, "");
    ENGINE_ASSERT(context::active() == ctx, "");
    ENGINE_ASSERT(this == ctx->get_scheduler(), "");
    ENGINE_ASSERT(ctx->is_context(type::worker_context), "");
    ENGINE_ASSERT(!ctx->is_context(type::pinned_context), "");
    ENGINE_ASSERT(!ctx->ready_is_linked(), "");
  ...
  void scheduler::yield(context *ctx) noexcept {
    ENGINE_ASSERT(nullptr != ctx, "");
    ENGINE_ASSERT(context::active() == ctx, "");
    ENGINE_ASSERT(ctx->is_context(type::worker_context) || ctx->is_context(type::main_context), "");
    ENGINE_ASSERT(!ctx->ready_is_linked(), "");
    ENGINE_ASSERT(!ctx->remote_ready_is_linked(), "");
    ENGINE_ASSERT(!ctx->sleep_is_linked(), "");
    ENGINE_ASSERT(!ctx->terminated_is_linked(), "");
  ...
  bool scheduler::wait_until(context *ctx, std::chrono::steady_clock::time_point const &sleep_tp) noexcept {
    ENGINE_ASSERT(nullptr != ctx, "");
    ENGINE_ASSERT(context::active() == ctx, "");
    ENGINE_ASSERT(ctx->is_context(type::worker_context) || ctx->is_context(type::main_context), "");
    ENGINE_ASSERT(!ctx->ready_is_linked(), "");
  ...
  bool scheduler::wait_until(
    context *ctx,
    std::chrono::steady_clock::time_point const &sleep_tp,
    detail::spinlock_lock &lk, waker &&w
  ) noexcept {
    ENGINE_ASSERT(nullptr != ctx, "");
    ENGINE_ASSERT(context::active() == ctx, "");
    ENGINE_ASSERT(ctx->is_context(type::worker_context) || ctx->is_context(type::main_context), "");
  ...
  void scheduler::suspend(detail::spinlock_lock &lk) noexcept {
    // resume another context
    algo_->pick_next()->resume(lk);
  }
  ...
  void scheduler::set_algo(algo::algorithm::ptr_t algo) noexcept {
  ...
  void scheduler::attach_main_context(context *ctx) noexcept {
    ENGINE_ASSERT(nullptr != ctx, "");
  ...
  void scheduler::attach_dispatcher_context(Core::IntrusivePtr<context> ctx) noexcept {
    ENGINE_ASSERT(ctx, "");
  ...
  void scheduler::attach_worker_context(context *ctx) noexcept {
    ENGINE_ASSERT(nullptr != ctx, "");
  ...
  void scheduler::detach_worker_context(context *ctx) noexcept {
    ENGINE_ASSERT(nullptr != ctx, "");
    ENGINE_ASSERT(!ctx->ready_is_linked(), "");
    ENGINE_ASSERT(!ctx->remote_ready_is_linked(), "");
    ENGINE_ASSERT(!ctx->sleep_is_linked(), "");
