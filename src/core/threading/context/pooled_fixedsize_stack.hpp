...
namespace Core::Context {
  ...
  namespace detail {
    ...
    struct map_stack_allocator {
      ...
      static char *malloc(const size_type bytes) {
        ...
        if (mmap(block, bytes, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON | MAP_FIXED | MAP_STACK, -1, 0) ==
            MAP_FAILED) {
          std::free(block);
          return 0;
        }
  ...
  class basic_pooled_fixedsize_stack {
    private:
      class storage: RefCounted {
        ...
        public:
          ...
          stack_size_(stack_size), storage_(stack_size, next_size, max_size) {
            ENGINE_STATIC_ASSERT(traits_type::is_unbounded() || (traits_type::maximum_size() >= stack_size_));
          }
          ...
          void deallocate(stack_context &sctx) noexcept {
            ENGINE_STATIC_ASSERT(sctx.sp);
            ENGINE_STATIC_ASSERT(traits_type::is_unbounded() || (traits_type::maximum_size() >= sctx.size));
      ...
      RefCountedPtr<storage> storage_ = nullptr;
    ...
    public:
      ...
      ) noexcept: storage_(RefCountedPtr<storage>::create(stack_size, next_size, max_size)) {
      }
