#pragma once

// #include <boost/config.hpp>
#include <boost/predef.h>
#include <core/threading/fiber/detail/config.hpp>

#include "core/application.hpp"

#ifndef SYS_futex
#define SYS_futex SYS_futex_time64
#endif

#if ENGINE_PLATFORM_LINUX
extern "C" {
#include <linux/futex.h>
#include <sys/syscall.h>
}
#elif BOOST_OS_BSD_OPEN
extern "C" {
#include <sys/futex.h>
}
#elif BOOST_OS_WINDOWS
#include <windows.h>
#endif

namespace Core {
  namespace Fibers {
    namespace detail {
#if ENGINE_PLATFORM_LINUX || BOOST_OS_BSD_OPEN
      ENGINE_FORCE_INLINE int sys_futex(void *addr, std::int32_t op, std::int32_t x) {
#if BOOST_OS_BSD_OPEN
        return ::futex
        (
          static_cast<volatile uint32_t*>(addr),
          static_cast<int>(op),
          x,
          nullptr,
          nullptr
        );
#else
        return ::syscall(SYS_futex, addr, op, x, nullptr, nullptr, 0);
#endif
      }

      ENGINE_FORCE_INLINE int futex_wake(std::atomic<std::int32_t> *addr) {
        return 0 <= sys_futex(static_cast<void*>(addr), FUTEX_WAKE_PRIVATE, 1) ? 0 : -1;
      }

      ENGINE_FORCE_INLINE int futex_wait(std::atomic<std::int32_t> *addr, std::int32_t x) {
        return 0 <= sys_futex(static_cast<void*>(addr), FUTEX_WAIT_PRIVATE, x) ? 0 : -1;
      }
#elif ENGINE_PLATFORM_WINDOWS
      ENGINE_FORCE_INLINE int futex_wake(std::atomic<std::int32_t> *addr) {
        ::WakeByAddressSingle(static_cast<void*>(addr));
        return 0;
      }

ENGINE_FORCE_INLINE int futex_wait(std::atomic<std::int32_t> *addr, std::int32_t x) {
        ::WaitOnAddress(static_cast<volatile void*>(addr), &x, sizeof(x), INFINITE);
        return 0;
      }
#else
# warn "no futex support on this platform"
#endif
    }
  }
}

#endif // BOOST_FIBERS_DETAIL_FUTEX_H
