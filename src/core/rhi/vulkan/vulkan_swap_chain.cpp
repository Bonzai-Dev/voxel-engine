...
namespace Core::RHI {
  ...
  VulkanSwapChain::~VulkanSwapChain() {
    ...
    //   vkDestroyImage(device, texture, device.getAllocationCallbacks());

    if (swapChain)
      vkDestroySwapchainKHR(device, swapChain, device.getAllocationCallbacks());

    if (surface)
      vkDestroySurfaceKHR(device, surface, device.getAllocationCallbacks());
  }
  ...
  Result VulkanSwapChain::create(const SwapChainInfo &swapChainInfo) {
     ...
     {
       SDL_Vulkan_CreateSurface(static_cast<SDL_Window *>(swapChainInfo.windowHandle), device, device.getAllocationCallbacks(), &surface);
     ...
     bool allowLowLatency = device.getInfo().features.lowLatency &&
     ...
     VkSurfaceFormat2KHR surfaceFormat = {VK_STRUCTURE_TYPE_SURFACE_FORMAT_2_KHR};
     {
       ...
       std::vector<VkSurfaceFormat2KHR> surfaceFormats(formatNum);
       for (uint32_t i = 0; i < formatNum; i++)
         surfaceFormats[i] = {VK_STRUCTURE_TYPE_SURFACE_FORMAT_2_KHR};

       VULKAN_CHECK(vkGetPhysicalDeviceSurfaceFormats2KHR(device, &surfaceInfo, &formatNum, surfaceFormats.data()));
       ...
       switch (swapChainInfo.format) {
         case SwapChainFormat::BT709_G10_16BIT:
           std::sort(surfaceFormats.data() + 0, surfaceFormats.data() + formatNum,
     ...
     */
...

     {
       ...
       // Create
       VULKAN_CHECK(vkCreateSwapchainKHR(device, &swapchainInfo, device.getAllocationCallbacks(), &swapChain));
     }
     ...
     // Textures
     {
       ...
       std::vector<VkImage> imageHandles(imageNum);
       vkGetSwapchainImagesKHR(device, swapChain, &imageNum, imageHandles.data());
     ...
     // Latency fence
     if (allowLowLatency) {
       latencyFence = std::make_unique<VulkanFence>(device);
     ...
     if (!device.getInfo().features.waitableSwapChain)
