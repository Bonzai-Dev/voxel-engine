...
#endif
#include <core/threading/context/fiber.hpp>
...
namespace Core::Fibers {
  ...
  class context {
    private:
      ...
      struct fss_data {
        ...
        fss_data(void *vp_, detail::fss_cleanup_function::ptr_t fn) noexcept: vp(vp_), cleanup_function(std::move(fn)) {
          ENGINE_STATIC_ASSERT(cleanup_function, "");
        }
    ...
    private:
      ...
      Core::Context::fiber c_{};
      std::chrono::steady_clock::time_point tp_;
      type type_;
      launch policy_;

      context(std::size_t initial_count, type t, launch policy) noexcept: use_count_{initial_count},
                                                                          tp_{
    ...
    public:
      class id {
        ...
        public:
          ...
          explicit id(context *impl) noexcept: impl_{impl} {
          }
          ...
          <<(std::basic_ostream<charT, traitsT> &os, id const &other) {
      ...
      friend bool operator==(context const &lhs, context const &rhs) noexcept {
        return &lhs == &rhs;
      }
      ...
      Core::Context::fiber suspend_with_cc() noexcept;

      Core::Context::fiber terminate() noexcept;
      ...
      void worker_link(List &lst) noexcept {
        static_assert(std::is_same<typename List::value_traits::hook_type, detail::worker_hook>::value,
                      "not a worker-queue");
      ...
      void ready_link(List &lst) noexcept {
        static_assert(std::is_same<typename List::value_traits::hook_type, detail::ready_hook>::value,
                      "not a ready-queue");
      ...
      void remote_ready_link(List &lst) noexcept {
        static_assert(std::is_same<typename List::value_traits::hook_type, detail::remote_ready_hook>::value,
                      "not a remote-ready-queue");
      ...
      void sleep_link(Set &set) noexcept {
        static_assert(std::is_same<typename Set::value_traits::hook_type, detail::sleep_hook>::value,
                      "not a sleep-queue");
      ...
      void terminated_link(List &lst) noexcept {
        static_assert(std::is_same<typename List::value_traits::hook_type, detail::terminated_hook>::value,
                      "not a terminated-queue");
        ENGINE_ASSERT(!terminated_is_linked(), "");
        lst.push_back(*this);
      }
      ...
      friend void intrusive_ptr_release(context *ctx) noexcept {
        ...
        if (1 == ctx->use_count_.fetch_sub(1, std::memory_order_release)) {
          ...
          Core::Context::fiber c = std::move(ctx->c_);
          // destruct context
          ctx->~context();
          // deallocated stack
          std::move(c).resume();
  ...
  inline bool operator<(context const &l, context const &r) noexcept {
  ...
  class worker_context final: public context {
    private:
      ...
      Core::Context::fiber run_(Core::Context::fiber &&c) {
        {
          // fn and tpl must be destroyed before calling terminate()
          auto fn = std::move(fn_);
          auto arg = std::move(arg_);
          Core::ignore_unused(c);
#if defined(BOOST_NO_CXX17_STD_APPLY)
          Core::Context::detail::apply(std::move(fn), std::move(arg));
#else
    ...
    public:
      ...
      worker_context(launch policy,
                     ...
                     Core::Context::preallocated const &palloc, StackAlloc &&salloc,
                     Fn &&fn, Arg... arg): context{1, type::worker_context, policy},
      ...
      worker_context(launch policy,
                     Core::Context::preallocated const &palloc, StackAlloc &&salloc,
                     Fn &&fn, Arg... arg): worker_context(policy, palloc, salloc, nullptr, std::forward<Fn>(fn),
                                                          std::forward<Arg>(arg)...) {
      }
  };
  ...
  static RefCountedPtr<context> make_worker_context_with_properties(launch policy,
                                                                    fiber_properties *properties,
                                                                    StackAlloc &&salloc,
                                                                    Fn &&fn, Arg... arg) {
    ...
    return RefCountedPtr<context>{
      new(storage) context_t{
        policy,
        properties,
        Core::Context::preallocated{storage, size, sctx},
  ...
  template <typename StackAlloc, typename Fn, typename... Arg>
  static RefCountedPtr<context> make_worker_context(launch policy,
                                                    StackAlloc &&salloc,
                                                    Fn &&fn, Arg... arg) {
...
namespace std {
  template <>
  struct hash<::Core::Fibers::context::id> {
    std::size_t operator()(::Core::Fibers::context::id const &id) const noexcept;
  };
}
