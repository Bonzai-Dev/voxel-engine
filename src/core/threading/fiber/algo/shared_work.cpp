...
namespace Core {
  namespace Fibers {
    namespace algo {
      //[awakened_ws
      void shared_work::awakened(context *ctx) noexcept {
      ...
      context *shared_work::pick_next() noexcept {
        ...
        if (!rqueue_.empty()) {
          ...
          rqueue_.pop_front();
          lk.unlock();
          ENGINE_STATIC_ASSERT(nullptr != ctx, "");
      ...
      void shared_work::suspend_until(std::chrono::steady_clock::time_point const &time_point) noexcept {
        if (suspend_) {
