...
#include <core/threading/fiber/detail/futex.hpp>
            ...
            while (retries++ < BOOST_FIBERS_RETRY_THRESHOLD) {
              ...
              if (0 != (expected = value_.load(std::memory_order_relaxed))) {
                ...
                std::uniform_int_distribution<std::int32_t> distribution{
                  0,
                  static_cast<std::int32_t>(1) << (std::min)(
                    collisions, static_cast<std::int32_t>(BOOST_FIBERS_CONTENTION_WINDOW_THRESHOLD))
                };
