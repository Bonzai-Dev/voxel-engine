    ...
    namespace detail {
      ...
      struct map_stack_allocator {
        ...
        static char *malloc(const size_type bytes) {
          ...
          if (mmap(block, bytes, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON | MAP_FIXED | MAP_STACK, -1,
                   0) == MAP_FAILED) {
            std::free(block);
            return 0;
          }
    ...
    class basic_pooled_fixedsize_stack {
      private:
        class storage {
          ...
          public:
            storage(std::size_t stack_size, std::size_t next_size, std::size_t max_size): use_count_(0),
