...
namespace boost::fibers {
  ...
  namespace algo {
    class algorithm: public Core::RefCounted {
      public:
        typedef Core::IntrusivePtr<algorithm> ptr_t;
        ...
        unsigned long release() override {
          unsigned long result = referenceCount.fetch_sub(1, std::memory_order_release);
          if (result == 1) {
    ...
    struct algorithm_with_properties: public algorithm_with_properties_base {
      ...
      void awakened(context *ctx) noexcept final {
        ...
        if (nullptr == props) [[likely]] {
          ...
          ENGINE_ASSERT(dynamic_cast<PROPS*>(props),
