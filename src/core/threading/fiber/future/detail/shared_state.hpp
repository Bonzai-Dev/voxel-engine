...
#include <core/threading/fiber/future/future_status.hpp>
    ...
    namespace detail {
      class shared_state_base {
        ...
        public:
          ...
          // TODO: implement this in our own intrusive pointer
      ...
      class shared_state: public shared_state_base {
        ...
        public:
          typedef RefCountedPtr<shared_state> ptr_type;
      ...
      class shared_state<void>: public shared_state_base {
        private:
          inline void set_value_(std::unique_lock<mutex> &lk) {
            ENGINE_ASSERT(lk.owns_lock(), "");
            if (ready_) [[unlikely]] {
              throw promise_already_satisfied();
            }
          ...
          inline void get_(std::unique_lock<mutex> &lk) {
            ENGINE_ASSERT(lk.owns_lock(), "");
            wait_(lk);
            if (except_) {
              std::rethrow_exception(except_);
