#include <core/threading/fiber/numa/algo/work_stealing.hpp>
#include <cmath>
#include <random>
  ...
  namespace Fibers {
    namespace numa {
      namespace algo {
        ...
        std::vector<std::uint32_t> get_local_cpus(std::uint32_t node_id,
                                                  std::vector<Core::Fibers::numa::node> const &topo) {
          for (auto &node : topo) {
            if (node_id == node.id) {
        ...
        std::vector<std::uint32_t> get_remote_cpus(std::uint32_t node_id,
                                                   std::vector<Core::Fibers::numa::node> const &topo) {
        ...
        void work_stealing::init_(std::vector<Core::Fibers::numa::node> const &topo,
                                  std::vector<RefCountedPtr<work_stealing>> &schedulers) {
          ...
          std::vector<RefCountedPtr<work_stealing>>{max_cpu_id + 1, nullptr}.swap(schedulers);
        }
        ...
        work_stealing::work_stealing(
          ...
          Core::Fibers::numa::pin_thread(cpu_id_);
