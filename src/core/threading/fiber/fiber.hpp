...
#include <core/threading/fiber/detail/disable_overload.hpp>
  ...
  namespace Fibers {
    class fiber {
      private:
        ...
        using ptr_t = RefCountedPtr<context>;
      ...
      public:
        ...
        >
#if BOOST_COMP_GNUC < 50000000
        fiber(launch policy, fiber_properties *properties, std::allocator_arg_t, StackAllocator &&salloc, Fn &&fn,
              Arg &&... arg):
#else
          fiber(launch policy, fiber_properties * properties, std::allocator_arg_t, StackAllocator && salloc, Fn && fn,
                Arg... arg) :
#endif
          impl_{
            make_worker_context_with_properties(policy, properties, std::forward<StackAllocator>(salloc),
                                                std::forward<Fn>(fn), std::forward<Arg>(arg)...)
          } {
          start_();
        }
        ...
        PROPS &properties() {
          ...
          ENGINE_ASSERT(props, "fiber::properties not set");
