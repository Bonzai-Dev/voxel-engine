...
namespace std {
  template <>
  struct is_error_code_enum<Core::Fibers::future_errc>: public true_type {
  };

  inline
  std::error_code make_error_code(Core::Fibers::future_errc e) noexcept {
    return std::error_code{static_cast<int>(e), Core::Fibers::future_category()};
  }

  inline
  std::error_condition make_error_condition(Core::Fibers::future_errc e) noexcept {
    return std::error_condition{static_cast<int>(e), Core::Fibers::future_category()};
  }
}
