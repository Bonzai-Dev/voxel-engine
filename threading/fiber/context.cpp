...
namespace boost {
  namespace fibers {
    ...
    boost::context::fiber context::suspend_with_cc() noexcept {
      context *prev = this;
      // context_initializer::active_ will point to `this`
      // prev will point to previous active context
      std::swap(context_initializer::active_, prev);
    ...
    boost::context::fiber context::terminate() noexcept {
      // protect for concurrent access
      std::unique_lock<detail::spinlock> lk{splk_};
      // mark as terminated
      terminated_ = true;
...
namespace std {
  std::size_t hash<::boost::fibers::context::id>::operator()(::boost::fibers::context::id const &id) const noexcept {
