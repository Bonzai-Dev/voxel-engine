...
namespace Core {
  namespace Fibers {
    ...
    class buffered_channel {
      ...
      public:
        explicit buffered_channel(std::size_t capacity):
          capacity_{capacity} {
          if (2 > capacity_ || 0 != (capacity_ & (capacity_ - 1))) [[unlikely]] {
            throw fiber_error{
    ...
    template <typename T>
    typename buffered_channel<T>::iterator begin(buffered_channel<T> &chan) {
      return typename buffered_channel<T>::iterator(&chan);
    }

    template <typename T>
    typename buffered_channel<T>::iterator end(buffered_channel<T> &) {
