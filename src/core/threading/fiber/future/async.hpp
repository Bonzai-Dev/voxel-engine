...
#include <core/threading/fiber/future/packaged_task.hpp>
#include <core/threading/fiber/policy.hpp>

namespace Core {
  namespace Fibers {
#if (defined(ENGINE_COMPILER_MSVC) && (_MSC_VER >= 1911 && _MSVC_LANG >= 201703)) || __cplusplus >= 202002L
    template <typename>
    struct result_of;
    ...
    > async(Policy policy, std::allocator_arg_t, StackAllocator salloc, Fn &&fn, Args... args) {
      ...
      future<result_type> f{pt.get_future()};
      fiber{
        policy, std::allocator_arg, salloc,
