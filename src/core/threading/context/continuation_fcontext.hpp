...
namespace Core::Context::detail {
  inline transfer_t context_unwind(transfer_t t) {
  ...
  template<typename Rec> void context_entry(transfer_t t) noexcept {
    // transfer control structure to the context-stack
    Rec *rec = static_cast<Rec *>(t.data);
    ENGINE_ASSERT(nullptr != t.fctx, "");
    ENGINE_STATIC_ASSERT(nullptr != rec);
    try {
  ...
  fcontext_t create_context1(StackAlloc &&salloc, Fn &&fn) {
    ...
    ENGINE_STATIC_ASSERT(ss_base != -1);
...
class continuation {
  private:
    ...
    continuation(detail::fcontext_t fctx) noexcept: fctx_{fctx} {
    }
  ...
  public:
    ...
    ~continuation() {
      if (nullptr != fctx_) {
        Core::Context::detail::ontop_fcontext(
#if defined(BOOST_NO_CXX14_STD_EXCHANGE)
...
#endif
          nullptr,
          Core::Context::detail::context_unwind);
      }
    }
    ...
    continuation resume() && {
      ENGINE_STATIC_ASSERT(nullptr != fctx_);
      return {
        Core::Context::detail::jump_fcontext(
#if defined(BOOST_NO_CXX14_STD_EXCHANGE)
          detail::exchange(fctx_, nullptr),
#else
          std::exchange(fctx_, nullptr),
...
#endif
          &p,
          Core::Context::detail::context_ontop<continuation, Fn>).fctx
      };
    }
...
continuation callcc(Fn &&fn) {
  return callcc(
    std::allocator_arg, Core::Context::fixedsize_stack(),
    std::forward<Fn>(fn));
}

template<typename StackAlloc, typename Fn>
continuation callcc(std::allocator_arg_t, StackAlloc &&salloc, Fn &&fn) {
  using Record = Core::Context::detail::record<continuation, StackAlloc, Fn>;
  return continuation{
    Core::Context::detail::create_context1<Record>(
...
continuation callcc(std::allocator_arg_t, Core::Context::preallocated palloc, StackAlloc &&salloc, Fn &&fn) {
  using Record = Core::Context::detail::record<continuation, StackAlloc, Fn>;
  return continuation{
    Core::Context::detail::create_context2<Record>(
...
template<typename Fn>
continuation callcc(std::allocator_arg_t, segmented_stack, Fn &&);

template<typename StackAlloc, typename Fn>
continuation callcc(std::allocator_arg_t, preallocated, segmented_stack, Fn &&);
#endif
