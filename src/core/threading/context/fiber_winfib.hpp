...
namespace Core::Context::detail {
  ...
  struct fiber_activation_record {
    ...
    fiber_activation_record() noexcept {
      ...
      fiber = ::ConvertThreadToFiber(nullptr);
      if (nullptr == fiber) {
        ENGINE_STATIC_ASSERT(ERROR_ALREADY_FIBER == ::GetLastError());
        fiber = ::GetCurrentFiber();
    ...
    fiber_activation_record(stack_context sctx_) noexcept: sctx{sctx_},
                                                           main_ctx{false} {
    }
  ...
  struct fiber_activation_record_initializer {
  ...
  struct forced_unwind {
    ...
    explicit forced_unwind(fiber_activation_record *from_): from{from_} {
    }
  };
  ...
  class fiber_capture_record: public fiber_activation_record {
    ...
    public:
      fiber_capture_record(stack_context sctx, StackAlloc &&salloc,
                           Fn &&fn) noexcept: fiber_activation_record(sctx),
      ...
      void run() {
...
#if defined(BOOST_NO_CXX17_STD_INVOKE)
          c = Core::Context::detail::invoke(fn_, std::move(c));
#else
        ...
        force_unwind = false;
        std::move(c).resume();
        ENGINE_ASSERT(false, "fiber already terminated");
      }
  };
  ...
  static fiber_activation_record *create_fiber1(StackAlloc &&salloc, Fn &&fn) {
    ...
    ENGINE_STATIC_ASSERT((sizeof(capture_t)) < sctx.size);
...
class fiber {
  private:
    ...
    fiber(detail::fiber_activation_record *ptr) noexcept: ptr_{ptr} {
    }
  ...
  public:
    ...
    fiber(std::allocator_arg_t, StackAlloc &&salloc, Fn &&fn): ptr_{
      Core::Context::detail::create_fiber1<fiber>(
    ...
    template<typename StackAlloc, typename Fn>
    fiber(std::allocator_arg_t, preallocated palloc, StackAlloc &&salloc, Fn &&fn): ptr_{
      Core::Context::detail::create_fiber2<fiber>(
    ...
    ~fiber() {
      if (nullptr != ptr_ && !ptr_->main_ctx) {
        if (!ptr_->terminated) {
          ...
          ptr_->resume();
          ENGINE_STATIC_ASSERT(ptr_->terminated);
        }
        ptr_->deallocate();
      }
    }
    ...
    fiber resume() && {
      ENGINE_STATIC_ASSERT(nullptr != ptr_);
#if defined(BOOST_NO_CXX14_STD_EXCHANGE)
...
#endif
      if (detail::fiber_activation_record::current()->force_unwind) {
        throw detail::forced_unwind{ptr};
      } else if (nullptr != detail::fiber_activation_record::current()->ontop) {
...
#if defined(BOOST_EMBTC)
    template<typename charT, class traitsT>
    inline std::basic_ostream<charT, traitsT> &operator<<(std::basic_ostream<charT, traitsT> &os, fiber const &other) {
      if (nullptr != other.ptr_) {
        return os << other.ptr_;
      } else {
        return os << "{not-a-context}";
    ...
    inline void swap(fiber &l, fiber &r) noexcept {
      l.swap(r);
    }
